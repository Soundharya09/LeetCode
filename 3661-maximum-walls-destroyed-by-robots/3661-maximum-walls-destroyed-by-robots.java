class Solution {
    public int maxWalls(int[] robots, int[] distance, int[] walls) {
        int n = robots.length;
        int[] left = new int[n], right = new int[n], num = new int[n];
        ...
        for (int i = 0; i < n; i++) {
            ...
            else leftPos = lowerBound(walls,robots[i] - robotsToDistance.get(robots[i]));
            left[i] = pos1 - leftPos;
            int rightPos = 0;
            if (i < n - 1) {
            ...
            else rightPos = upperBound(walls,robots[i] + robotsToDistance.get(robots[i]));
